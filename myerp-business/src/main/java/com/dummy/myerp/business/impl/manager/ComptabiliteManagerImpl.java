...
public class ComptabiliteManagerImpl extends AbstractBusinessManager implements ComptabiliteManager {
	...
	public synchronized void addReference(EcritureComptable pEcritureComptable) throws FunctionalException, NumberFormatException, NotFoundException {
		...
		/*
		 * Mettre à jour la référence de l'écriture avec la référence calculée
		 * (RG_Compta_5)
		 */
		String updateReference = pEcritureComptable.getJournal().getCode() + "-" + annee + "/"
		 ...
		 */
		if (numero == 1) {
			getDaoProxy().getComptabiliteDao().insertSequenceEcritureComptable(codeJournal, sequenceJournal);
		} else {
